...
def train_one_epoch(model, dataloader, criterion, optimizer, scaler, device, epoch):
    ...
    for images, masks in pbar:
        ...
        with autocast(enabled=config.USE_AMP):
            ...
            # Simple loss without brain mask
        ...
        if has_nan:
            ...
            optimizer.zero_grad()
            scaler.update()  # CRITICAL: Update scaler to reset state
            continue
...
def validate(model, dataloader, criterion, device, epoch):
    ...
    with torch.no_grad():
        ...
        for images, masks in pbar:
            ...
            # Forward pass (no AMP in validation for stability)
            outputs = model(images)
            # Simple loss without brain mask
            loss = criterion(outputs, masks)
...
def main():
    ...
    train_dataset.dataset.transform = get_train_transform(
        aug_p_horizontal=config.AUG_HORIZONTAL_FLIP,
        ...
        aug_p_grid=0.0  # Not in config, use default
    )
    ...
    # V0.3: Learning rate scheduler (CosineAnnealingWarmRestarts)
    # Fixed V0.2 issue: ReduceLROnPlateau caused LR to drop to 0
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer,
        T_0=50,        # First restart period
        T_mult=1,      # Period multiplier (1 = constant period)
        eta_min=1e-6   # Minimum LR (won't drop to 0)
    )
